    ...
    while ((option = getopt(argc, argv, OPTIONS)) != -1) {
        switch (option) {
            ...
            case 'h':
                printf(
                    "SYNOPSIS\n"
                    "   Generates public and private keys for the S-S "
                    "cryptosystem.\n"
                    "\n"
                    "USAGE\n"
                    "   %s [-b:i:n:d:s:vh] [-b bits] [-i iters] [-n "
                    "public_key_file] "
                    "[-d private_key_file] [-s seed]\n"
                    "\n"
                    "OPTIONS\n"
                    "   -b bits               Specify the number of bits for "
                    "the public modulus (default: 10).\n"
                    "   -i iterations         Number of Miller-Rabin primality "
                    "test iterations (default: 50).\n"
                    "   -n public_key_file    Path to the public key file "
                    "(limit : 100 words) "
                    "(default: ss.pub).\n"
                    "   -d private_key_file   Path to the private key file "
                    "(limit : 100 words)"
                    "(default: ss.priv).\n"
                    "   -s seed               Random seed for initialization "
                    "(default: UNIX time).\n"
